...
class VGG_multi_scale(nn.Module):
    def __init__(self, features, num_classes=1000, init_weights=True):
        super(VGG_multi_scale, self).__init__()
        self.features = features
        self.avgpool = nn.AdaptiveAvgPool2d((7, 7))
        self.classifier = nn.Sequential(
            nn.Linear(512 * 7 * 7, 4096),
...
class VGG_FCN_multi_scale(nn.Module):
    def __init__(self, features, num_classes=1000, init_weights=True):
        super(VGG_FCN_multi_scale, self).__init__()
        self.features = features
        self.avgpool = nn.AdaptiveAvgPool2d((7, 7))
        self.classifier = nn.Sequential(
            # nn.Linear(512 * 7 * 7, 4096),
...
def vgg16_multi_scale(batch_norm=False, num_classes=1000):
    model = VGG_multi_scale(make_layers(cfgs['D']), num_classes=num_classes)
    return model
...
def vgg16_FCN_multi_scale(batch_norm=False, num_classes=1000):
    model = VGG_FCN_multi_scale(make_layers(cfgs['D']), num_classes=num_classes)
    return model
...
def get_vgg(model, num_classes=1000):
    ...
    if model == "vgg16_multi_scale":
        return vgg16_multi_scale(num_classes=num_classes)

    if model == 'vgg16_FCN':
        return vgg16_FCN(num_classes=num_classes)

    if model == 'vgg16_FCN_multi_scale':
        return vgg16_FCN_multi_scale(num_classes=num_classes)
