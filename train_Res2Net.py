...
class trainer(nn.Module):
    ...
    def net_init(self):
        try:
            ...
            # self.net = Res2Net121(num_classes=10).to(device)
            self.model_optim_lr = torch.load("./saved_models/Res2Net/bs" + str(self.config.batch_size) + "_lr" + str(self.config.learning_rate).split('.')[1] + "_" +  str(self.config.model_name) + ".pth")
            ...
            self.state = torch.load("./saved_models/Res2Net/bs" + str(self.config.batch_size) + "_lr" + str(self.config.learning_rate).split('.')[1] + "_" +  str(self.config.model_name) + "_State" + ".pth")
    ...
    def train(self):
        ...
        for i in pbar:
            ...
            torch.save(self.model_optim_lr, "./saved_models/Res2Net/bs" + str(self.config.batch_size) + "_lr" + str(self.config.learning_rate).split('.')[1] + "_" +  str(self.config.model_name) + ".pth")

            self.state['epoch'] = i+1
            torch.save(self.state, "./saved_models/Res2Net/bs" + str(self.config.batch_size) + "_lr" + str(self.config.learning_rate).split('.')[1] + "_" +  str(self.config.model_name) + "_State" + ".pth")
    ...
    def plot(self):
        plot_loss_accuracy("./saved_models/Res2Net/bs" + str(self.config.batch_size) + "_lr" + str(self.config.learning_rate).split('.')[1] + "_" +  str(self.config.model_name) + "_State" + ".pth", "./plots/Res2Net/losses/bs" + str(self.config.batch_size) + "_lr" + str(self.config.learning_rate).split('.')[1] + "_" +  str(self.config.model_name) + ".jpg", self.trainset, self.testset)
...
def get_config():
    ...
    parser.add_argument('--model_name', type=str, default='res2net50_26w_6s_dev')
